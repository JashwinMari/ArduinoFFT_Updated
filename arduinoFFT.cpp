...
arduinoFFT::arduinoFFT(double *vReal, double *vImag, uint32_t samples, double samplingFrequency)
{// Constructor
	this->_vReal = vReal;
	this->_vImag = vImag;
...
void arduinoFFT::Compute(double *vReal, double *vImag, uint32_t samples, uint8_t dir)
{
	#warning("This method is deprecated and may be removed on future revisions.")
...
void arduinoFFT::Compute(double *vReal, double *vImag, uint32_t samples, uint8_t power, uint8_t dir)
{	// Computes in-place complex-to-complex FFT
	// Reverse bits
	#warning("This method is deprecated and may be removed on future revisions.")
	uint32_t j = 0;
...
void arduinoFFT::ComplexToMagnitude(double *vReal, double *vImag, uint32_t samples)
{	// vM is half the size of vReal and vImag
	#warning("This method is deprecated and may be removed on future revisions.")
	for (uint32_t i = 0; i < samples; i++) {
...
void arduinoFFT::DCRemoval(double *vData, uint32_t samples)
{
	// calculate the mean of vData
	#warning("This method is deprecated and may be removed on future revisions.")
	double mean = 0;
...
void arduinoFFT::Windowing(double *vData, uint32_t samples, uint8_t windowType, uint8_t dir)
{// Weighing factors are computed once before multiple use of FFT
// The weighing function is symetric; half the weighs are recorded
	#warning("This method is deprecated and may be removed on future revisions.")
...
double arduinoFFT::MajorPeak(double *vD, uint32_t samples, double samplingFrequency)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	double maxY = 0;
	uint32_t IndexOfMaxY = 0;
	//If sampling_frequency = 2 * max_frequency in signal,
	//value would be stored at position samples/2
	for (uint32_t i = 1; i < ((samples >> 1) + 1); i++) {
...
void arduinoFFT::MajorPeak(double *vD, uint32_t samples, double samplingFrequency, double *f, double *v)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	double maxY = 0;
	uint32_t IndexOfMaxY = 0;
